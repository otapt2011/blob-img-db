...
export default async function handler(req, res) {
  ...
  if (action === 'start') {
    ...
    console.log('token available:', !!BLOB_READ_WRITE_TOKEN);
    ...
    try {
      ...
      console.log('start result:', multipart);
      return res.status(200).json({ uploadId: multipart.uploadId });
    } catch (err) {
      console.error('createMultipartUpload error:', err);
  ...
  if (action === 'uploadPart') {
    ...
    try {
      ...
      console.log('uploadPart options:', options);
      ...
      console.log('uploadPart result:', part);
